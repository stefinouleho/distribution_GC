...
//printf("fin atomes\n");
	...
	for (l=0 ; l<M.nb_liaisons ; l++){
		...
		//printf(" %d %d %d\n", l,M.liste_liaisons[l].A1,M.liste_liaisons[l].A2);
...
void tailles_molecules(struct molecule *M){
	...
	for(nb_mol=0;nb_mol<90130;nb_mol++){
		if((M[nb_mol].nb_atomes -M[nb_mol].nb_hydrogene)  > max) max = M[nb_mol].nb_atomes - M[nb_mol].nb_hydrogene;
	}
	int tableau[max + 1];
	for(i=0;i<max + 1 ;i++) tableau[i] = 0;
	...
	for(i=0;i<max + 1 ;i++) fprintf(F,"%d %d \n",i,tableau[i]);
	fclose(F);
